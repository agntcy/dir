...
| **P1 — Personal registry** | `core/v2` + `artifact/v2` + `naming/v2` protos (incl. the **Entry/Collection** data model); ArtifactService + NamingService over the existing OCI store; `dirctl artifact` + `dirctl tag`/`name` + `dirctl collection`; the Artifact DAG design doc. | Devs use it solo, like `git init` before GitHub. Success: repeat solo usage. |
...
20. **Collection membership defaults**: should `collection create` pin members by digest by default (frozen, lockfile-like) with `--follow` opt-in for name refs, or the reverse?
21. **AI Catalog conformance target**: which conformance level (1–3 per ai-catalog.io) should `catalog export` produce by default, and do we contribute dir-specific extensions upstream?
