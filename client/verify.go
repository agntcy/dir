...
func (c *Client) Verify(ctx context.Context, req *signv1.VerifyRequest) (*signv1.VerifyResponse, error) {
	...
	for _, sig := range signatures {
		...
		case *signv1.VerifyRequestProvider_Any:
			signerInfo, verifyErr = VerifySignature(ctx, payload, publicKeys, sig)
...
// VerifySignature verifies a single signature against the record payload (record CID).
func VerifySignature(ctx context.Context, payload []byte, publicKeys []string, sig *signv1.Signature) (*signv1.SignerInfo, error) {
	if len(sig.GetContentBundle()) == 0 {
		signerInfo, err := cosign.VerifyWithKeys(ctx, payload, publicKeys, sig)
		if err != nil {
	...
	signerInfo, err := cosign.VerifyWithOIDC(
		payload,
...
// Used when returning server-cached verification results.
//
//nolint:cyclop
