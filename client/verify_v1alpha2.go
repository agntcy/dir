...
func (c *Client) VerifyOIDCv2(_ context.Context, req *signtypes.VerifyOIDCRequest) (*emptypb.Empty, error) {
	agent := req.GetRecord()
...
func (c *Client) VerifyWithKeyv2(_ context.Context, req *signtypes.VerifyWithKeyRequest) (*emptypb.Empty, error) {
	// Validate request.
	...
	if req.GetRecord().GetSignature() == nil {
		return &emptypb.Empty{}, errors.New("agent has no signature")
	}

	// Extract signature data from the agent.
	sigBundleRawJSON, err := base64.StdEncoding.DecodeString(req.GetRecord().GetSignature().GetContentBundle())
	if err != nil {
		return &emptypb.Empty{}, fmt.Errorf("failed to decode signature: %w", err)
	}
