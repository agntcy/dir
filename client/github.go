...
// githubPerRPCCredentials implements credentials.PerRPCCredentials for GitHub PAT authentication.
type githubPerRPCCredentials struct {
	token string
}

// GetRequestMetadata attaches the GitHub PAT to the request metadata as a Bearer token.
...
// newGitHubCredentials creates a new PerRPCCredentials that injects GitHub PAT.
