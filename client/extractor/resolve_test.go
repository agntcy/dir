...
func TestResolveExtractorRemoteReturnsRemoteBackend(t *testing.T) {
	// A configured remote address resolves to the remote backend without dialing
	// (the gRPC client connects lazily), so no server is needed here.
	ext, err := ResolveExtractor(Config{RemoteAddr: "127.0.0.1:31234"})
...
func TestResolveExtractorNeitherErrors(t *testing.T) {
	_, err := ResolveExtractor(Config{AssetDir: filepath.Join(t.TempDir(), "absent")})
