...
func TestSignAndVerify_RoundTrip(t *testing.T) {
	...
	sig := ownershipv1.NewClaim(testSpiffeID)

	if err := ownershipv1.SignClaim(sig, keyPEM, certPEM); err != nil {
	...
	if err := ownershipv1.VerifyClaim(sig, nil); err != nil {
...
func TestVerifyClaim_TamperedSignature(t *testing.T) {
	...
	// Corrupt the Base64-encoded signature by flipping a character.
	raw := sig.GetSignature()
	if len(raw) > 0 {
		b := []byte(raw)
		b[0] ^= 0x01
...
func TestVerifyClaim_TamperedOwnerID(t *testing.T) {
	...
	sig.Annotations["owner_id"] = "spiffe://example.org/agent/attacker"
...
func TestSignClaim_IdentityMismatch(t *testing.T) {
	...
	// Claim owner is bob but cert SAN is alice — should fail before signing.
	sig := ownershipv1.NewClaim("spiffe://example.org/agent/bob")
...
func TestIsOwnershipClaim(t *testing.T) {
	sig := ownershipv1.NewClaim(testSpiffeID)

	if !ownershipv1.IsOwnershipClaim(sig) {
		t.Fatal("IsOwnershipClaim should be true for a claim created by NewClaim")
	}
}
...
func TestGetOwnerID(t *testing.T) {
	...
	if got := ownershipv1.GetOwnerID(sig); got != testSpiffeID {
		t.Fatalf("GetOwnerID = %q, want %q", got, testSpiffeID)
	}
}
