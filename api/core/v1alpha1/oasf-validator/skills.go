...
const OASFEndpoint = "https://schema.oasf.agntcy.org/api/classes"
...
func (sv *SkillValidator) Validate(skills []*corev1alpha1.Skill) error {
	...
	for _, skill := range skills {
		if skill.GetClassUid() == "" {
			errorMessages = append(errorMessages, "skill class UID is required")
			continue
		}

		if !sv.HasSkill(skill.GetClassUid()) {
			errorMessages = append(errorMessages, skill.GetClassUid())
		}
	...
	if len(errorMessages) > 0 {
		return fmt.Errorf("skills with uid %s not found", errorMessages)
	}
