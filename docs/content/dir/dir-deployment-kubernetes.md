...
- **Startup takes about 40 seconds, and needs the OASF schema URL reachable.** From image `v1.3.0` the embedding model ships inside the image, so nothing is fetched from `huggingface.co` — the pull is ~90 MB instead. On start the server converts the model, fetches the taxonomy from `OASF_SDK_EXTRACTOR_OASF_URL`, and embeds the catalog before going ready. A new OASF release is picked up by restarting the pod.
