// Package runtime provides the Kubernetes runtime adapter.
package runtime
...
// KubernetesAdapter implements the Adapter interface for Kubernetes.
...
func NewKubernetesAdapter(cfg *config.KubernetesConfig) (*KubernetesAdapter, error) {
...
func (k *KubernetesAdapter) RuntimeType() models.Runtime {
	return models.RuntimeKubernetes
}
...
func (k *KubernetesAdapter) ListWorkloads(ctx context.Context) ([]*models.Workload, error) {
...
func (k *KubernetesAdapter) WatchEvents(ctx context.Context, eventChan chan<- *models.WorkloadEvent) error {
	labelSelector := k.labelKey + "=" + k.labelValue
...
func (k *KubernetesAdapter) watchPods(ctx context.Context, labelSelector string, eventChan chan<- *models.WorkloadEvent) {
	for {
		select {
		case <-ctx.Done():
			return
		...
		for event := range watcher.ResultChan() {
			...
			case watch.Modified:
				if pod.Status.Phase == corev1.PodRunning {
					eventChan <- &models.WorkloadEvent{Type: models.EventTypeModified, Workload: workload}
...
func (k *KubernetesAdapter) watchServices_(ctx context.Context, eventChan chan<- *models.WorkloadEvent) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (k *KubernetesAdapter) updatePodsForService(ctx context.Context, svc *corev1.Service, eventChan chan<- *models.WorkloadEvent) {
...
func (k *KubernetesAdapter) getServicesByNamespace(ctx context.Context) map[string][]*corev1.Service {
...
func (k *KubernetesAdapter) findServicesForPod(pod *corev1.Pod, services []*corev1.Service) []*corev1.Service {
...
func (k *KubernetesAdapter) podToWorkload(pod *corev1.Pod, services []*corev1.Service) *models.Workload {
	labels := pod.Labels
	if labels == nil {
	...
	return &models.Workload{
		...
		Name:            pod.Name,
		Hostname:        hostname,
		Runtime:         models.RuntimeKubernetes,
		WorkloadType:    models.WorkloadTypePod,
