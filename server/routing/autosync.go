...
var autosyncLogger = logging.Logger("routing/autosync")
...
// autosyncJob is a unit of work: pull+ingest the announced record from a peer.
...
// autosyncManager pulls records (and their referrers) announced by trusted peers
// over libp2p and ingests them locally with full parity to a normal push.
//
...
type autosyncManager struct {
	...
	queue    chan autosyncJob
	inFlight map[string]struct{}
	mu       sync.Mutex
}

func newAutosyncManager(
	allowSet map[peer.ID]struct{},
	transport recordFetcher,
	router peerRouter,
	ingestor ingest.Ingestor,
	...
	return &autosyncManager{
		...
		store:     store,
		validator: validator,
		queue:     make(chan autosyncJob, AutosyncQueueSize),
		inFlight:  make(map[string]struct{}),
	}
}

// start launches the bounded worker pool. Workers stop when ctx is cancelled.
func (m *autosyncManager) start(ctx context.Context, wg *sync.WaitGroup) {
	for range AutosyncWorkerCount {
		wg.Go(func() {
			m.worker(ctx)
		})
	}

	autosyncLogger.Info("Autosync workers started",
		"workers", AutosyncWorkerCount,
...
func (m *autosyncManager) maybeEnqueue(notif *handlerSync) {
	if notif == nil || notif.Ref == nil {
		return
	}
	...
	if _, ok := m.allowSet[notif.Peer.ID]; !ok {
		return
	}
	...
	select {
	case m.queue <- autosyncJob{ref: notif.Ref, peer: notif.Peer}:
		autosyncLogger.Debug("Autosync job enqueued", "cid", cid, "peer", notif.Peer.ID)
	default:
		// Queue full: release the in-flight marker and drop.
		m.clearInFlight(cid)
		autosyncLogger.Warn("Autosync queue full, dropping announcement", "cid", cid, "peer", notif.Peer.ID)
	}
}

func (m *autosyncManager) clearInFlight(cid string) {
	m.mu.Lock()
	delete(m.inFlight, cid)
	m.mu.Unlock()
}

func (m *autosyncManager) worker(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		case job := <-m.queue:
			m.process(ctx, job)
			m.clearInFlight(job.ref.GetCid())
		}
	}
}
...
// item.
func (m *autosyncManager) process(parentCtx context.Context, job autosyncJob) {
	cid := job.ref.GetCid()
	peerID := job.peer.ID

	// Dedupe against local storage: if we already have the record, skip the pull.
	if _, err := m.store.Lookup(parentCtx, job.ref); err == nil {
		autosyncLogger.Debug("Record already present locally, skipping autosync", "cid", cid, "peer", peerID)

		return
	}

	// Bound the whole job so a slow/unreachable peer cannot tie up the worker.
	ctx, cancel := context.WithTimeout(parentCtx, AutosyncJobTimeout)
	defer cancel()

	record, err := m.pullRecord(ctx, peerID, job.ref, job.peer.Addrs)
	if err != nil {
		autosyncLogger.Error("Autosync failed to pull record", "cid", cid, "peer", peerID, "error", err)
	...
	if actual := record.GetCid(); actual != cid {
		autosyncLogger.Warn("Autosync rejected record: CID mismatch",
	...
	if err != nil {
		autosyncLogger.Error("Autosync record validation error", "cid", cid, "peer", peerID, "error", err)

		return
	}

	if !valid {
		autosyncLogger.Warn("Autosync rejected record: OASF validation failed",
	...
	if _, err := m.ingestor.ImportRecord(ctx, record); err != nil {
		autosyncLogger.Error("Autosync failed to ingest record", "cid", cid, "peer", peerID, "error", err)

		return
	}

	autosyncLogger.Info("Autosync ingested record", "cid", cid, "peer", peerID)

	m.syncReferrers(ctx, peerID, job.ref)
}
...
func (m *autosyncManager) pullRecord(ctx context.Context, peerID peer.ID, ref *corev1.RecordRef, addrs []ma.Multiaddr) (*corev1.Record, error) {
	if len(addrs) > 0 {
	...
	autosyncLogger.Debug("Autosync pull failed, re-resolving peer via DHT", "peer", peerID, "error", err)
...
func (m *autosyncManager) syncReferrers(ctx context.Context, peerID peer.ID, recordRef *corev1.RecordRef) {
	descriptors, err := m.transport.ListReferrers(ctx, peerID, recordRef)
	if err != nil {
		autosyncLogger.Warn("Autosync failed to list referrers", "cid", recordRef.GetCid(), "peer", peerID, "error", err)
	...
	for _, desc := range descriptors {
		if _, ok := allowedReferrerTypes[desc.Type]; !ok {
			autosyncLogger.Warn("Autosync skipped referrer: disallowed type",
		...
		if err != nil {
			autosyncLogger.Warn("Autosync failed to pull referrer",
		...
		if !m.verifyReferrer(referrer, recordRef.GetCid(), desc) {
			autosyncLogger.Warn("Autosync rejected referrer: verification failed",
		...
		if _, err := m.ingestor.ImportReferrer(ctx, recordRef.GetCid(), referrer); err != nil {
			autosyncLogger.Warn("Autosync failed to ingest referrer",
				"cid", recordRef.GetCid(), "peer", peerID, "referrer_cid", desc.Cid, "error", err)

			continue
		}

		autosyncLogger.Debug("Autosync ingested referrer",
			"cid", recordRef.GetCid(), "peer", peerID, "referrer_cid", desc.Cid, "type", desc.Type)
	}
}
...
func (m *autosyncManager) verifyReferrer(referrer *corev1.RecordReferrer, recordCID string, desc rpc.ReferrerDescriptor) bool {
	if referrer == nil {
		return false
	}
