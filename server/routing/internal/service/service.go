...
func New(host host.Host, protocol protocol.ID, listenCh chan<- string, ignored []peer.AddrInfo) *Service {
	return &Service{
		host:     host,
		protocol: protocol,
		listenCh: listenCh,
...
func (s *Service) StartMessaging(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			s.Echo(ctx, fmt.Sprintf("Message: Hello from %s", s.host.ID().String()))
		}
	}
}
