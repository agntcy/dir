...
Record validation is configured as a YAML list. Each entry names a provider, its settings, and the operations it runs on (`push`, `autosync`, `index`). An empty list disables record validation. This list cannot be set via environment variables.
