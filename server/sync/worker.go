...
func (w *Worker) processWorkItem(ctx context.Context, item WorkItem) {
	...
	var finalStatus storev1alpha2.SyncStatus

	if err != nil {
		logger.Error("Sync failed", "worker_id", w.id, "sync_id", item.SyncID, "error", err)

		finalStatus = storev1alpha2.SyncStatus_SYNC_STATUS_FAILED
	} else {
		logger.Info("Sync completed successfully", "worker_id", w.id, "sync_id", item.SyncID)

		finalStatus = storev1alpha2.SyncStatus_SYNC_STATUS_COMPLETED
	}
...
//
//nolint:unparam
func (w *Worker) performSync(_ context.Context, item WorkItem) error {
