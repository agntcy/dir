...
type StoreAPI interface {
	...
	Push(context.Context, string, io.ReadCloser) (*corev1.ObjectRef, error)

	// Pull record from content store
	Pull(context.Context, *corev1.ObjectRef) (*corev1.ObjectMeta, io.ReadCloser, error)

	// Lookup metadata about the record from reference
	Lookup(context.Context, *corev1.ObjectRef) (*corev1.ObjectMeta, error)

	// Delete the record
	Delete(context.Context, *corev1.ObjectRef) error

	// Walk walks records individually
	Walk(ctx context.Context, head *corev1.ObjectRef, walkFn func(*corev1.ObjectMeta) error, walkOpts ...func()) error
