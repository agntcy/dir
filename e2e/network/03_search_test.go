...
var _ = ginkgo.Describe("Running dirctl end-to-end tests for remote routing search with OR logic", func() {
	...
	ginkgo.Context("domain search tests", func() {
		ginkgo.It("should find record by domain name", func() {
			// Test domain search functionality - record has "life_science/biotechnology" domain
			output := cli.Routing().Search().
				WithDomain("life_science/biotechnology"). // Query for exact domain
				WithMinScore(1).
				WithLimit(10).
				OnServer(utils.Peer2Addr).
				ShouldSucceed()

			ginkgo.GinkgoWriter.Printf("=== DOMAIN SEARCH TEST (exact match) ===\n%s", output)

			// Should find the record
			gomega.Expect(output).To(gomega.ContainSubstring(cid))
			ginkgo.GinkgoWriter.Printf("✅ SUCCESS: Domain search found record with exact domain match")
		})

		ginkgo.It("should find record by domain pattern with wildcard", func() {
			// Test domain search with wildcard pattern
			output := cli.Routing().Search().
				WithDomain("*biotechnology"). // Query with wildcard
				WithMinScore(1).
				WithLimit(10).
				OnServer(utils.Peer2Addr).
				ShouldSucceed()

			ginkgo.GinkgoWriter.Printf("=== DOMAIN SEARCH TEST (wildcard) ===\n%s", output)

			// Should find the record
			gomega.Expect(output).To(gomega.ContainSubstring(cid))
			ginkgo.GinkgoWriter.Printf("✅ SUCCESS: Domain search found record with wildcard pattern")
		})

		ginkgo.It("should find record combining domain and skill filters", func() {
			// Test combining domain with skill search (AND logic between different filter types)
			output := cli.Routing().Search().
				WithDomain("life_science/biotechnology").                                             // Domain filter
				WithSkill("natural_language_processing/natural_language_generation/text_completion"). // Skill filter
				WithMinScore(2).                                                                      // Need both filters to match
				WithLimit(10).
				OnServer(utils.Peer2Addr).
				ShouldSucceed()

			ginkgo.GinkgoWriter.Printf("=== DOMAIN + SKILL SEARCH TEST ===\n%s", output)

			// Should find the record since both domain and skill match
			gomega.Expect(output).To(gomega.ContainSubstring(cid))
			ginkgo.GinkgoWriter.Printf("✅ SUCCESS: Combined domain+skill search found record")
		})

		ginkgo.It("should not find record with non-existent domain", func() {
			// Test that non-matching domain returns no results
			output := cli.Routing().Search().
				WithDomain("nonexistent/domain"). // Domain that doesn't exist
				WithMinScore(1).
				WithLimit(10).
				OnServer(utils.Peer2Addr).
				ShouldSucceed()

			ginkgo.GinkgoWriter.Printf("=== DOMAIN SEARCH TEST (non-existent) ===\n%s", output)

			// Should NOT find any records
			gomega.Expect(output).To(gomega.ContainSubstring("No remote records found"))
			gomega.Expect(output).NotTo(gomega.ContainSubstring(cid))
			ginkgo.GinkgoWriter.Printf("✅ SUCCESS: Non-existent domain correctly returned no results")
		})
	})
