...
var _ = ginkgo.Describe("Running dirctl end-to-end tests using a network multi peer deployment", func() {
	...
	ginkgo.It("should discover remote records via routing search", func() {
		...
		// This tests the new network discovery functionality
		output := cli.Routing().Search().
			WithSkill("Natural Language Processing").
			WithLimit(10).
			OnServer(utils.Peer2Addr).ShouldSucceed()

		// Should show search results or indicate search completion
		gomega.Expect(output).To(gomega.ContainSubstring("remote record"))

		// Note: Depending on DHT propagation timing, we might or might not find results
