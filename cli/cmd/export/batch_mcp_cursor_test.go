...
func TestMCPCursorBatchFormatter(t *testing.T) {
	bf := getBatchFormatter("mcp-cursor")
	require.NotNil(t, bf, "mcp-cursor should have a batch formatter")

	t.Run("merges multiple records into single mcp.json", func(t *testing.T) {
		dir := t.TempDir()
		r1 := newMCPCursorTestRecord(t, testMCPCursorRecordJSON)
