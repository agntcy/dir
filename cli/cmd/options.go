...
func init() {
	...
	flags.StringVar(&config.Client.AuthMode, "auth-mode", config.Client.AuthMode, "Authentication mode: jwt, tls, oidc, insecure, none, or empty for auto-detect")
	flags.StringVar(&config.Client.SpiffeSocketPath, "spiffe-socket-path", config.Client.SpiffeSocketPath, "Path to SPIFFE Workload API socket (for JWT authentication)")
