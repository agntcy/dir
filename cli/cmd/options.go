...
func init() {
	...
	flags.StringVar(&config.Client.AuthMode, "auth-mode", config.Client.AuthMode, "Authentication mode: x509, jwt, jwt-tls, token (SPIFFE), tls, oidc, insecure, none, or empty for auto-detect")
	flags.StringVar(&config.Client.SpiffeSocketPath, "spiffe-socket-path", config.Client.SpiffeSocketPath, "Path to SPIFFE Workload API socket (for x509, jwt, or jwt-tls authentication)")
	...
	flags.StringVar(&config.Client.JWTAudience, "jwt-audience", config.Client.JWTAudience, "JWT audience (for jwt or jwt-tls authentication mode)")
