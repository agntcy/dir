...
func init() {
	...
	// MCP Scanner flags
	flags.StringSliceVar(&opts.Scanner.Modes, "mcp-scanner-modes", nil, "MCP scanner modes to run (e.g. behavioral)")
	flags.DurationVar(&opts.Scanner.Timeout, "mcp-scanner-timeout", mcpscannerconfig.DefaultTimeout, "Timeout per record scan")
	flags.StringVar(&opts.Scanner.CLIPath, "mcp-scanner-cli-path", mcpscannerconfig.DefaultCLIPath, "Path to mcp-scanner binary (default: mcp-scanner from PATH)")
	flags.BoolVar(&opts.Scanner.FailOnError, "mcp-scanner-fail-on-error", mcpscannerconfig.DefaultFailOnError, "Do not import records that have error-severity scanner findings")
	flags.BoolVar(&opts.Scanner.FailOnWarning, "mcp-scanner-fail-on-warning", mcpscannerconfig.DefaultFailOnWarning, "Do not import records that have warning-severity scanner findings")
