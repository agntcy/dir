...
- AND logic: All specified queries must match
...
func runSearchCommand(cmd *cobra.Command) error {
	...
	queries := make([]*routingv1.RecordQuery, 0, len(searchOpts.Skills)+len(searchOpts.Locators))
	...
	if len(queries) == 0 {
		presenter.Printf(cmd, "No search criteria specified. Use --skill or --locator flags.\n")
		presenter.Printf(cmd, "Example: dirctl routing search --skill 'AI' --locator 'docker-image'\n")
