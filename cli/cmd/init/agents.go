...
Step 3 — DIR skill
Wire this Directory into your AI coding agents: the DIR skill (a usage guide)
for push, search, and pull records. Content is built in — no Directory
connection is made. Writes are idempotent and atomic.
`
...
// installable artifacts (the DIR skill). No Directory round-trip.
...
// then (interactively) ask which of them get the DIR skill — installing it
// via the shared agentinstall engine. selectAgents is the chooser (injected).
func installAgents(cmd *cobra.Command, env agentcfg.Env, opts *options, selectAgents agentSelector) error {
	...
	// Interactive: prompt once, pre-selecting all candidates.
