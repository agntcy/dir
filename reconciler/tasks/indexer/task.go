...
func NewTask(cfg reconcilercfg.Indexer, localRegistry config.Registry, store types.StoreAPI, repo registry.TagLister, db types.SearchDatabaseAPI, validator corev1.Validator) (*Task, error) {
	return &Task{
		config:       cfg,
		db:           db,
		store:        store,
