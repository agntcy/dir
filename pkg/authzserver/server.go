...
func (s *AuthorizationServer) detectProvider(httpReq *authv3.AttributeContext_HttpRequest, token string) string {
	...
	// GitHub tokens
	if strings.HasPrefix(token, "ghp_") || // Personal Access Token
		strings.HasPrefix(token, "gho_") || // OAuth token
	...
	// Could add: if strings.HasPrefix(token, "eyJ") && len(token) > minJWTLength { return "azure" }
