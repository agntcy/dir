...
func TestProvider_Name(t *testing.T) {
	...
	if got := provider.Name(); got != authprovider.ProviderGithub {
		t.Errorf("Name() = %v, want %v", got, authprovider.ProviderGithub)
	}
}
...
func TestProvider_ValidateToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr && identity.Provider != authprovider.ProviderGithub {
				t.Errorf("ValidateToken() identity.Provider = %v, want %v", identity.Provider, authprovider.ProviderGithub)
			}
		})
	}
}
